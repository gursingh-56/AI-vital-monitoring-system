import{GoogleGenAI as m}from"@google/genai";export const config={runtime:"nodejs"};const a=(e,t,n)=>{e.statusCode=t,e.setHeader("Content-Type","application/json"),e.end(JSON.stringify(n))};async function g(e){if(e.body!==void 0&&e.body!==null&&e.body!=="")return typeof e.body=="string"?JSON.parse(e.body):e.body;const t=[];for await(const s of e)t.push(s);const n=Buffer.concat(t).toString("utf8");return n?JSON.parse(n):{}}const o=e=>typeof e=="number"&&Number.isFinite(e);function h(e){if(typeof e!="object"||e===null)return null;const t=e,n=t.bloodPressure;return!o(t.heartRate)||!o(t.bloodSugar)||!o(t.spo2)||!o(t.temperature)||typeof n!="object"||n===null||!o(n.systolic)||!o(n.diastolic)?null:{heartRate:t.heartRate,bloodSugar:t.bloodSugar,spo2:t.spo2,temperature:t.temperature,bloodPressure:{systolic:n.systolic,diastolic:n.diastolic}}}const p=(e,t)=>`
...
`;export default async function y(e,t){if(e.method!=="POST")return a(t,405,{error:"Method not allowed"});const n=process.env.GEMINI_API_KEY;if(!n)return console.error("GEMINI_API_KEY is not set on the server"),a(t,500,{error:"Analysis service is not configured."});let s;try{s=await g(e)}catch{return a(t,400,{error:"Invalid JSON body."})}const{vitals:d,age:r}=s??{},i=h(d);if(!i)return a(t,400,{error:"A complete set of numeric vital signs is required."});const u=o(r)?r:void 0;try{const c=await new m({apiKey:n}).models.generateContent({model:"gemini-2.5-flash",contents:p(i,u)});return a(t,200,{text:c.text})}catch(l){return console.error("Error generating analysis from Gemini:",l),a(t,502,{error:"The AI service is unavailable. Please try again later."})}}
